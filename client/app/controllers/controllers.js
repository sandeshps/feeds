'use strict';
  ...
  angular.module('feedsApp')
    ...
    .controller('LoginCtrl', function ($scope, $timeout, $cookies, User, $location) {
      ...
      $scope.login = function () {
        ...
        Stamplay.User
          .login(credentials, function(err, res) {
            if(err) {
              console.log(err);
              $scope.error = true;
              $timeout(function () {
    ...
    .controller('Admin.RulesCtrl', function ($scope, Admin, $timeout, $route, $cookies) {
      ...
      $scope.saveKeyValue = function () {
        if($('#search_handle .text').text().toLowerCase().trim() === 'search')
          key = 'search';
        else
          key = $('#handler_source #hsource').text().toLowerCase();
        ...
        if(record.toLowerCase().trim() === 'all'){
          ...
          for(var index=1; index<keyValues.length; index++) {
            ...
            if(key === 'search') {
              ...
              Admin.fetchDataFromApi(key, value)
                .then(function (response) {
                  console.log(response);
